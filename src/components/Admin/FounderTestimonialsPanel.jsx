...
const toPayload = (item) => ({ quote: item.quote, authorName: item.authorName, authorRole: item.authorRole });

// Testimonios de clientes o colegas — se muestran en /portafolio solo si hay
// al menos uno cargado. No se inventan por defecto: los agrega el fundador.
const FounderTestimonialsPanel = () => {
  const { authFetch } = useAuth();
  const { items, loading, refresh } = useContentItems('/api/founder-testimonials');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId === 'new') {
        ...
        await authFetch('/api/founder-testimonials', { method: 'POST', body: JSON.stringify({ ...form, sortOrder: nextOrder }) });
      } else {
        const current = items.find(i => i.id === editingId);
        await authFetch(`/api/founder-testimonials/${editingId}`, { method: 'PUT', body: JSON.stringify({ ...form, sortOrder: current?.sortOrder ?? 0 }) });
      }
      await refresh();
      closeForm();
    } catch (err) {
  ...
  const handleDelete = async (item) => {
    if (!window.confirm(`¿Eliminar el testimonio de "${item.authorName}"?`)) return;
    try {
      await authFetch(`/api/founder-testimonials/${item.id}`, { method: 'DELETE' });
      await refresh();
    } catch (err) {
  ...
  const move = async (index, direction) => {
    ...
    try {
      await Promise.all([
        authFetch(`/api/founder-testimonials/${a.id}`, { method: 'PUT', body: JSON.stringify({ ...toPayload(a), sortOrder: b.sortOrder }) }),
        authFetch(`/api/founder-testimonials/${b.id}`, { method: 'PUT', body: JSON.stringify({ ...toPayload(b), sortOrder: a.sortOrder }) }),
      ]);
      await refresh();
    } catch (err) {
      window.alert(err.message);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-black uppercase text-gray-900 dark:text-white">Testimonios</h2>
        <button
          onClick={openCreate}
        ...
        <form onSubmit={handleSubmit} className="border-2 border-gray-900 dark:border-white bg-white dark:bg-[#0f0f12] p-6 flex flex-col gap-4 max-w-xl">
          ...
          <label className="flex flex-col gap-1 text-[11px] font-mono uppercase text-gray-500">
            ...
            <textarea
              ...
              required
              rows={3}
              placeholder="Lo que dijo tu cliente o colega..."
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <label className="flex flex-col gap-1 text-[11px] font-mono uppercase text-gray-500">
              Rol / empresa (ej. Dueño, Restaurante La Casa)
              <input
      ...
      <div className="border-2 border-gray-900 dark:border-white bg-white dark:bg-[#0f0f12] overflow-x-auto">
        {loading ? (
          ...
          <p className="p-6 text-gray-500 font-mono text-[13px]">Todavía no hay testimonios. Agrega el primero.</p>
        ) : (
          <table className="w-full text-left">
            <thead>
              <tr className="border-b-2 border-gray-900 dark:border-white text-[10px] font-mono uppercase tracking-widest text-gray-500">
                ...
                <th className="px-4 py-3">Testimonio</th>
            ...
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id} className="border-b border-gray-200 dark:border-white/10 text-[13px] text-gray-900 dark:text-white">
                  ...
                  <td className="px-4 py-3 font-bold whitespace-nowrap">{item.authorName}</td>
                  <td className="px-4 py-3 text-gray-500 max-w-md truncate">{item.quote}</td>
                  <td className="px-4 py-3">
...
export default FounderTestimonialsPanel;
