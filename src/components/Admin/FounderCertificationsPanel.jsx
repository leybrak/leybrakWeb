...
const EMPTY_FORM = { name: '', issuer: '', issuerLogoUrl: '', certificateLink: '', dateEarned: '' };
...
const toPayload = (form) => ({
  name:            form.name,
  issuer:          form.issuer,
  issuerLogoUrl:   form.issuerLogoUrl || null,
  certificateLink: form.certificateLink || null,
  dateEarned:      form.dateEarned,
});

// Certificaciones del fundador — se muestran en /portafolio.
const FounderCertificationsPanel = () => {
  const { authFetch } = useAuth();
  const { items: certs, loading, refresh } = useContentItems('/api/founder-certifications');
  ...
  const openEdit = (c) => { setForm(toFormState(c)); setEditingId(c.id); setError(''); };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId === 'new') {
        const nextOrder = certs.length > 0 ? Math.max(...certs.map(c => c.sortOrder ?? 0)) + 1 : 1;
        await authFetch('/api/founder-certifications', { method: 'POST', body: JSON.stringify({ ...toPayload(form), sortOrder: nextOrder }) });
      } else {
        const current = certs.find(c => c.id === editingId);
        await authFetch(`/api/founder-certifications/${editingId}`, { method: 'PUT', body: JSON.stringify({ ...toPayload(form), sortOrder: current?.sortOrder ?? 0 }) });
      }
      await refresh();
      closeForm();
    } catch (err) {
  ...
  const handleDelete = async (c) => {
    ...
    try {
      await authFetch(`/api/founder-certifications/${c.id}`, { method: 'DELETE' });
      await refresh();
    } catch (err) {
  ...
  const move = async (index, direction) => {
    ...
    if (otherIndex < 0 || otherIndex >= certs.length) return;
    const a = certs[index];
    const b = certs[otherIndex];
    setReordering(true);
    try {
      await Promise.all([
        authFetch(`/api/founder-certifications/${a.id}`, { method: 'PUT', body: JSON.stringify({ ...toPayload(toFormState(a)), sortOrder: b.sortOrder }) }),
        authFetch(`/api/founder-certifications/${b.id}`, { method: 'PUT', body: JSON.stringify({ ...toPayload(toFormState(b)), sortOrder: a.sortOrder }) }),
      ]);
      await refresh();
    } catch (err) {
      window.alert(err.message);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-black uppercase text-gray-900 dark:text-white">Certificaciones</h2>
        <button
          onClick={openCreate}
      ...
      {editingId && (
        <form onSubmit={handleSubmit} className="border-2 border-gray-900 dark:border-white bg-white dark:bg-[#0f0f12] p-6 flex flex-col gap-4 max-w-xl">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <label className="flex flex-col gap-1 text-[11px] font-mono uppercase text-gray-500">
              Fecha (texto libre, ej. Marzo 2026)
              <input
                value={form.dateEarned}
                onChange={e => handleChange('dateEarned', e.target.value)}
                className="bg-transparent border-2 border-gray-300 dark:border-white/20 focus:border-leybrak-blue text-gray-900 dark:text-white px-3 py-2.5 text-[13px] font-mono outline-none normal-case"
              />
            </label>
          </div>
          ...
          <label className="flex flex-col gap-1 text-[11px] font-mono uppercase text-gray-500">
            Logo del emisor (URL, opcional)
            <input
              value={form.issuerLogoUrl}
              onChange={e => handleChange('issuerLogoUrl', e.target.value)}
              placeholder="https://..."
          ...
          <label className="flex flex-col gap-1 text-[11px] font-mono uppercase text-gray-500">
            Enlace al certificado (opcional)
            <input
      ...
      <div className="border-2 border-gray-900 dark:border-white bg-white dark:bg-[#0f0f12] overflow-x-auto">
        {loading ? (
          ...
          <p className="p-6 text-gray-500 font-mono text-[13px]">Todavía no hay certificaciones. Agrega la primera.</p>
        ) : (
          <table className="w-full text-left">
            ...
            <tbody>
              {certs.map((c, i) => (
                <tr key={c.id} className="border-b border-gray-200 dark:border-white/10 text-[13px] text-gray-900 dark:text-white">
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <button
                        onClick={() => move(i, 1)}
                        disabled={i === certs.length - 1 || reordering}
                        className="text-gray-400 hover:text-leybrak-blue disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                        aria-label="Bajar"
                      >
                        <ArrowDown size={14} />
                  ...
                  <td className="px-4 py-3 font-mono text-gray-500">{c.issuer}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      <button onClick={() => openEdit(c)} className="text-gray-400 hover:text-leybrak-blue transition-colors">
                        <Pencil size={16} />
                      </button>
...
export default FounderCertificationsPanel;
